...
*DB:* `results/study.db`  *git:* `c2d9d5e41585627381277f3a490ff31d59afcd62`
...
> *50 sitios recuperados vía Browserbase tras un bloqueo inicial del scraper local; ver `engines_used` en findings.json.*
...
Sitios con banner: **64**.

| CMP | count |
|---|---|
| <sin CMP reconocida> | 31 |
| OneTrust | 12 |
...
Banners observados (incluyendo los que un re-scrape posterior pudo no haber detectado): **68**. Cumplen la regla mínima: **14** (20.6% del total observado).

Top fallas (entre banners actualmente detectados):
| regla incumplida | count |
|---|---|
| no_reject_button | 43 |
| accept_more_prominent_than_reject | 11 |
| no_accept_button | 7 |
...
**453** sitios depositan cookies de tracking (categorías: advertising, analytics, marketing, social) en la primera carga.
...
*(Mostrando los primeros 50 de 453; ver `findings.json` para el listado completo.)*
